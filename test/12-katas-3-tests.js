...
describe("12-katas-3-tasks", function () {
  it.optional(
    "findStringInSnakingPuzzle shoud return true if word occurrs in the specified puzzle",
    () => {
      ...
      ["FUNCTION", "NULL", "EMBER", "HOISTING", "GIT", "ARENA"].forEach(
        (word) => {
          assert(
            !tasks.findStringInSnakingPuzzle(puzzle, word),
  ...
  it.optional(
    "getPermutations should return all possible string permutations",
    () => {
      [
        {
          chars: "a",
  ...
  it.optional(
    "getMostProfitFromStockQuotes should return the max profit from stock trading",
    () => {
      [
        {
          quotes: [1, 2, 3, 4, 5, 6],
  ...
  it.optional(
    "urlShortener should return encoded string shorter than original url",
    () => {
      [
        ...
        assert(
          ...
          `urlShortener.encode for "${data}" returns "${actual}" that is only ${
            data.length / actual.length
          } times less than original url`
        );
      });
    }
  );
  ...
  it.optional(
    "urlShortener should decode shorten link to to the original url",
    () => {
      [
        "https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Math/imul",
  ...
  var paths = ["task/12-katas-3-tasks.js"];
